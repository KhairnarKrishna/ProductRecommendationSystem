...
def load_data():
    ...
    for l in range(len(df)):
        data.append(
            ['/static/images/laptop.jpg', df.iloc[l]["Product"], df.iloc[l]["Price"], 'A16 Bionic Chip', '8 GB RAM',
             '512 GB SSD', 'NVIDIA GeForce RTX 2040', '13.6 Inch (2560 x 1664 Pixel)', '90 Hz refresh rate', 'Mac OS',
             94])
    return data
...
def process_input():
    ...
    for laptop in data:
        if min_budget <= laptop[2] <= max_budget:
            filtered_laptops.append(laptop)

    processed_result = (f"min-budget: {min_budget}\n"
                        f"max-budget: {max_budget}\n"
                        f"laptop-type: {laptop_type}\n"
                        f"LAPTOPS: {filtered_laptops}\n")
    return jsonify({'result': processed_result, 'data': filtered_laptops})
